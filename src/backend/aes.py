class AESBackend:
    ...
    def mix_columns(self, state):
        ...
        for i in range(4):
            ...
            state[i][0] = self.gmul(s0, 2) ^ self.gmul(s1, 3) ^ s2 ^ s3
            state[i][1] = s0 ^ self.gmul(s1, 2) ^ self.gmul(s2, 3) ^ s3
            state[i][2] = s0 ^ s1 ^ self.gmul(s2, 2) ^ self.gmul(s3, 3)
            state[i][3] = self.gmul(s0, 3) ^ s1 ^ s2 ^ self.gmul(s3, 2)
        return state
    ...
    def inv_mix_columns(self, state):
        ...
        for i in range(4):
            ...
            state[i][0] = self.gmul(s0, 0x0e) ^ self.gmul(s1, 0x0b) ^ self.gmul(s2, 0x0d) ^ self.gmul(s3, 0x09)
